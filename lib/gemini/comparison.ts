 ...
 * acknowledge BOTH the "found" and "skipped" cases. There is intentionally no
 * "found but no source" variant — that would be the hallucination state the
 * kickoff hardline forbids.
 */
...
export interface RunComparisonInput {
  ...
  /** Receipt/invoice image, optional. When present, Gemini sees the visual
   *  product, which materially improves match quality. */
  imageBase64?: string;
 ...
 * without Gemini ever being called; items where no real Barbados-local source
 * was returned get `{ kind: 'skipped', reason: 'no-source' }`.
 *
 * This is the function that enforces the kickoff non-negotiables in code:
 *   1. Never invent values. If Gemini returns nothing locally, the result is
 *      `no-source`, never a fabricated price.
 *   2. Sources must be cited. A "found" result requires a sourceUrl that
 *      (a) parses as a URL, (b) is Barbados-local per the locality check,
 *      and (c) appears in the call's groundingChunks (proves Gemini's
 *      Google-Search tool actually fetched it — model can't hallucinate a URL
 *      that wasn't in its retrieval set).
 *   3. Skip utilities entirely. Electricity/water descriptions don't even
 *      hit the wire.
 */
export async function runComparison(
  ...
  // Step 1: utility check — these never see Gemini.
  ...
  // Gemini actually returns. Default-skipped means "if anything goes wrong
  // from here, the result is blank, never a fabricated price".
  ...
  try {
    ...
    for (const c of chunks) {
      const web = c?.web;
      if (web?.uri) {
        groundingByUri.set(web.uri, { uri: web.uri, title: web.title ?? '' });
      }
    }
  ...
  for (const item of modelItems) {
    ...
    if (!lineItem) {
      // Model returned an itemIndex outside the input range — ignore.
      continue;
    }
    ...
    result.set(lineItem.id, {
      ...
      comparableProduct: String(item.comparableProduct ?? '').trim() || lineItem.description,
      comparableVendor: String(item.comparableVendor ?? '').trim() || (locality.hostname ?? ''),
      sourceUrl: item.sourceUrl,
      sourceTitle: groundingMeta?.title || String(item.comparableVendor ?? '').trim(),
      searchedAt,
    });
  }
...
function parseModelResponse(text: string | null | undefined): ModelComparisonItem[] {
  if (!text) return [];
