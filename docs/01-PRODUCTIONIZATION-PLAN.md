...
**Prototype baseline:** branch [`prototype-import`](https://github.com/JP-Billionz/Invoice-Receipt-Intelligence/tree/prototype-import) — unmodified AI Studio export, pushed alongside this PR.

This is the markdown-only Plan PR requested in the kickoff. It captures (1) what the
prototype does today, (2) the proposed structure for the Next.js port, and (3) open
questions Jamai or Claude (Cowork) needs to answer before code starts moving.
...
lib/
  ...
  tenant.ts                     # server-side tenant resolution helper
...
model Tenant {
  id        String   @id @default(cuid())
  name      String
  currency  String   @default("BBD")        // confirm — see open Q
  settings  Json?                            // COA mapping, default accounts, etc.
  users     User[]
  scans     Scan[]
  createdAt DateTime @default(now())
}
...
model Scan {
  ...
  fileName          String
  mimeType          String
  fileBlobUrl       String?    // null if image discarded after extraction — see open Q
  ...
  - type: web
    ...
    envVars:
      ...
      - key: SENDGRID_API_KEY
        sync: false
      - key: EMAIL_FROM                 # confirm format — see open Q
        sync: false
...
1. **App shell** — Next.js scaffold, Prisma + Neon connection, Auth.js v5
   magic-link, custom `createUser` → Tenant transaction. **First deploy to Render
   on this alone**, smoke-test login.
2. **Port the scanner** — `lib/gemini/extract.ts` from
   `services/geminiService.ts`; UI components into `app/(app)/scan/`. Move the
   journal-balance algorithm to `lib/accounting/balance.ts` (deduplicated).
3. **Persistence** — Prisma schema + first migration. POST `/api/scan` writes the
   Scan row + LineItems + JournalEntries in one transaction. Records list page.
4. **Local comparisons** — `lib/gemini/comparison.ts` per §2.3. Tests first, then
   wire into `POST /api/scan/[scanId]/comparison`. UI hides the comparison column
   for utility line items and shows "—" for `no-source` skips with a tooltip
   "Searched, no local Barbados source found".
5. **PWA** — manifest, service worker, camera capture verification. Lighthouse
   audit ≥ 90 PWA score.
6. **Internal pilot** — feed real AISB receipts through, tune extraction + COA
   mapping (depends on open questions §3.2 and §3.3).
...
## 4. What I will NOT do until this is approved

- No `package.json` rewrite. No Next.js scaffold. No Prisma migrations.
- No additional branches pushed.
- No changes to `main`.

Once Jamai approves (or marks specific sections for revision), I'll open a sequence
of small, focused PRs each tied to a numbered step in §2.6.
