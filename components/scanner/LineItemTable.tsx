...
import type { LineItemResponse } from '@/lib/scan/serialize';

interface LineItemTableProps {
  items: LineItemResponse[];
  /** Tenant currency fallback (Plan §4.6). */
  currency: string;
}

/**
 * Display-only render of a scan's line items. Ported from prototype
 * `components/LineItemTable.tsx` with currency formatting respecting the
 * tenant currency (was hardcoded en-US/USD in the prototype).
 */
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50">
                ...
                <th className="p-4 text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 text-center">
                  Qty
                </th>
                <th className="p-4 text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 text-right">
                  Unit Price
                </th>
                <th className="p-4 text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 text-right">
                  Total
