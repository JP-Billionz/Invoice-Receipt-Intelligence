...
// share the same instance. Vitest requires this pattern for cross-scope
// access into module mocks.
const { mockGenerateContent } = vi.hoisted(() => ({
  mockGenerateContent: vi.fn(),
}));

vi.mock('@google/genai', () => ({
  // Use a class so `new GoogleGenAI(...)` works under Vitest 4 (where
  // arrow-function implementations aren't constructable).
  GoogleGenAI: class {
...
// Helper to build the mocked Gemini response shape.
...
describe('runComparison — kickoff hardline guardrails', () => {
  // ============================================================================
  // The required test: "no source → blank, never invented".
  // ============================================================================
  describe('REQUIRED: no Barbados source returned → blank, never hallucinated', () => {
    ...
    it('returns no-source even when the model FABRICATES prices+URLs without grounding', async () => {
      ...
      mockGenerateContent.mockResolvedValueOnce(
        geminiResponse({
          ...
          groundingUris: [], // KEY: model claims a URL but search tool didn't return it
        }),
      );
    ...
    it('returns no-source when groundingChunks contains only NON-BB sources', async () => {
      mockGenerateContent.mockResolvedValueOnce(
        geminiResponse({
          ...
          groundingUris: [{ uri: 'https://amazon.com/dp/B0XYZ', title: 'Amazon' }],
        }),
      );
    ...
    it('returns no-source when Gemini itself throws', async () => {
  ...
  describe('Utilities exclusion — Gemini is never called', () => {
    it('skips electricity bill without invoking Gemini', async () => {
      const result = await runComparison({
        lineItems: [{ id: 'li-1', description: 'Electricity bill — May', total: 450 }],
        allowlist: ALLOWLIST,
      });
    ...
    it('mixed input: utilities skipped without Gemini seeing them, others go to Gemini', async () => {
      ...
      const promptText = call.contents.parts.find((p: any) => 'text' in p).text;
      // The prompt only contains the non-utility line item.
  ...
  // ============================================================================
  // The "found" path — every condition must hold
  // ============================================================================
  describe('Found path — accepts ONLY valid Barbados-local sources from grounding', () => {
    it('accepts a .bb-TLD source that appears in groundingChunks', async () => {
      ...
      const result = await runComparison({
        lineItems: [{ id: 'li-1', description: 'Office paper ream', total: 25 }],
        allowlist: ALLOWLIST,
      });
      ...
      if (r?.kind === 'found') {
        ...
        expect(r.sourceUrl).toBe('https://carltonbarbados.bb/product/a4-paper');
    ...
    it('accepts an allowlisted .com source (e.g. PriceSmart) when present in grounding', async () => {
      mockGenerateContent.mockResolvedValueOnce(
        geminiResponse({
          ...
          groundingUris: [
            { uri: 'https://bb.pricesmart.com/sku/12345', title: 'PriceSmart' },
          ],
        }),
      );
    ...
    it('rejects items pointing to out-of-range itemIndex', async () => {
      mockGenerateContent.mockResolvedValueOnce(
        geminiResponse({
          ...
          groundingUris: [{ uri: 'https://carltonbarbados.bb/x', title: 'x' }],
        }),
      );
  ...
  describe('Edge cases', () => {
    it('returns empty map for empty input, no Gemini call', async () => {
      const result = await runComparison({ lineItems: [], allowlist: ALLOWLIST });
